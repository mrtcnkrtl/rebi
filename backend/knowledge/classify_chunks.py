from __future__ import annotations

import json
import re
import os
import time
from typing import Optional

from google.genai import types

from config import get_logger
from knowledge.db import pg_conn
from rag_service import gemini_client, _gemini_response_text

log = get_logger("knowledge.classify")

def _exec(cur, sql: str, params=None):
    """
    Supabase Session Pooler (PgBouncer) may reuse server sessions.
    Force simple execution without server-side prepared statements.
    """
    if params is None:
        return cur.execute(sql, prepare=False)
    return cur.execute(sql, params, prepare=False)


def _norm_entity_name(x: str) -> str:
    s = (x or "").strip().lower()
    s = " ".join(s.split())
    return s[:120]


def _upsert_entity_links(
    *,
    user_id: str,
    folder_id: str | None,
    chunk_id: str,
    ingredients: list[str],
) -> None:
    names = [_norm_entity_name(v) for v in (ingredients or []) if _norm_entity_name(v)]
    if not names:
        return
    with pg_conn(autocommit=True) as conn:
        with conn.cursor() as cur:
            for name in sorted(set(names)):
                # upsert entity
                _exec(
                    cur,
                    """
                    insert into public.knowledge_entities (user_id, folder_id, name, kind)
                    values (%s, %s::uuid, %s, 'ingredient')
                    on conflict (user_id, folder_id, name) do update set name = excluded.name
                    returning id
                    """,
                    (user_id, folder_id, name),
                )
                entity_id = cur.fetchone()[0]
                # link
                _exec(
                    cur,
                    """
                    insert into public.knowledge_chunk_entities (chunk_id, entity_id, user_id, folder_id)
                    values (%s, %s, %s, %s::uuid)
                    on conflict (chunk_id, entity_id) do nothing
                    """,
                    (chunk_id, entity_id, user_id, folder_id),
                )


_INGREDIENT_TERMS = [
    # ── actives / acids ──────────────────────────────────────
    "niacinamide", "niasinamid",
    "adapalene", "adapalen",
    "azelaic", "azelaik",
    "salicylic", "salisilik", "bha",
    "glycolic", "glikolik",
    "lactic acid", "laktik asit",
    "mandelic", "mandelik",
    "malic acid", "malik asit",
    "tartaric acid", "tartarik asit",
    "ferulic", "ferulik",
    "phytic acid", "fitik asit",
    "gluconolactone", "glukonolakton",
    "lactobionic", "laktobiyonik",
    "succinic acid", "süksinik asit",
    "retinol", "retinal", "retinaldehyde", "retinaldehit",
    "tretinoin", "tretinoin",
    "retinyl palmitate", "retinil palmitat",
    "hydroxypinacolone", "granactive",
    "tranexamic", "traneksamik",
    "arbutin", "alpha arbutin",
    "kojic", "kojik",
    "hydroquinone", "hidrokinon",
    "panthenol", "pantenol",
    "ceramide", "seramid",
    "urea", "üre",
    "zinc pca", "çinko pca",
    "sulfur", "kükürt",
    "benzoyl peroxide", "benzoil peroksit",
    "resveratrol",
    "bakuchiol",
    "coenzyme q10", "koenzim q10", "ubiquinone", "ubikinon",
    "peptide", "peptid", "matrixyl", "argireline",
    "copper peptide", "bakır peptid",
    "ectoin", "ektoin",
    # ── vitamins / antioxidants ──────────────────────────────
    "vitamin c", "c vitamini", "ascorbic", "askorbik",
    "ascorbyl glucoside", "askorbil glukozit",
    "magnesium ascorbyl phosphate",
    "sodium ascorbyl phosphate",
    "ethyl ascorbic acid", "etil askorbik",
    "tetrahexyldecyl ascorbate",
    "vitamin e", "e vitamini", "tokoferol", "tocopherol",
    "vitamin a", "a vitamini",
    "vitamin b5", "b5 vitamini",
    "biotin", "biyotin",
    "ergothioneine", "ergotiyonein",
    "glutathione", "glutatyon",
    # ── humectants / barrier ─────────────────────────────────
    "hyaluronic", "hyaluronik",
    "sodium hyaluronate",
    "glycerin", "gliserin",
    "betaine", "betain",
    "trehalose", "trehaloz",
    "sodium pca",
    "propanediol", "propandiol",
    "butylene glycol",
    "squalane", "skualan",
    "squalene", "skualen",
    "petrolatum", "vazelin",
    "cholesterol", "kolesterol",
    "phytosphingosine", "fitosfingozin",
    "beta glucan", "beta glukan",
    "polyglutamic acid", "poliglutamik",
    "allantoin",
    "bisabolol",
    "madecassoside", "madecassosid",
    "centella", "cica", "gotu kola",
    # ── sunscreen filters ────────────────────────────────────
    "zinc oxide", "çinko oksit",
    "titanium dioxide", "titanyum dioksit",
    "avobenzone", "avobenzon",
    "octocrylene", "oktokrilen",
    "homosalate", "homosalat",
    "tinosorb",
    "uvinul",
    # ── carrier / botanical oils ─────────────────────────────
    "jojoba",
    "argan",
    "shea", "shea butter", "shea yağı",
    "cocoa butter", "kakao yağı",
    "almond oil", "badem yağı", "sweet almond",
    "coconut oil", "hindistan cevizi yağı",
    "olive oil", "zeytinyağı", "zeytin yağı",
    "sunflower oil", "ayçiçek yağı",
    "grapeseed", "üzüm çekirdeği yağı",
    "castor oil", "hint yağı",
    "sesame oil", "susam yağı",
    "avocado oil", "avokado yağı",
    "macadamia",
    "evening primrose", "çuha çiçeği",
    "borage oil", "hodan yağı",
    "hemp seed oil", "kenevir yağı",
    "safflower oil", "aspir yağı",
    "camellia oil", "kamelya yağı",
    "tamanu oil", "tamanu yağı",
    "marula oil", "marula yağı",
    "rosehip", "kuşburnu",
    "pomegranate seed oil", "nar çekirdeği yağı",
    "sea buckthorn", "yalancı iğde",
    "tea tree", "çay ağacı",
    "neem oil", "neem yağı",
    "argan oil",
    # ── essential oils / aromatics ───────────────────────────
    "lavender", "lavanta",
    "rosemary", "biberiye",
    "peppermint", "nane yağı",
    "eucalyptus", "okaliptus",
    "ylang ylang",
    "geranium", "sardunya",
    "frankincense", "günlük yağı",
    "bergamot",
    "sandalwood", "sandal ağacı",
    "patchouli", "paçuli",
    "clary sage",
    "thyme oil", "kekik yağı",
    # ── plant extracts / botanicals ──────────────────────────
    "aloe", "aloe vera", "aloe vera özü",
    "papatya", "chamomile",
    "green tea", "yeşil çay",
    "propolis",
    "licorice", "meyan kökü",
    "calendula", "aynısefa",
    "witch hazel", "cadı fındığı",
    "ginseng", "ginseng özü",
    "turmeric", "zerdeçal",
    "ginkgo biloba",
    "cucumber", "salatalık özü",
    "oat", "yulaf",
    "rice extract", "pirinç özü",
    "soya özü", "soybean",
    "willow bark", "söğüt kabuğu",
    "pine bark", "çam kabuğu",
    "grape seed extract", "üzüm çekirdeği özü",
    "pomegranate extract", "nar özü",
    "ginger extract", "zencefil özü",
    "horse chestnut", "at kestanesi",
    "marigold",
    "snail", "salyangoz",
    "bee venom", "arı zehiri",
    "honey", "bal özü",
    # ── ferments / probiotics ────────────────────────────────
    "galactomyces",
    "bifida ferment",
    "lactobacillus",
    "saccharomyces",
    # ── proteins / misc ──────────────────────────────────────
    "collagen", "kolajen",
    "elastin",
    "keratin", "keratin",
    "omega-3", "omega 3",
    "omega-6", "omega 6",
    "dimethicone", "dimetikon",
]


# TR / alternate spellings → canonical lowercase ASCII name used for the entity index.
_NORMALIZE_MAP: dict[str, str] = {
    "niasinamid": "niacinamide",
    "adapalen": "adapalene",
    "azelaik": "azelaic acid",
    "salisilik": "salicylic acid",
    "bha": "salicylic acid",
    "glikolik": "glycolic acid",
    "laktik asit": "lactic acid",
    "mandelik": "mandelic acid",
    "malik asit": "malic acid",
    "tartarik asit": "tartaric acid",
    "ferulik": "ferulic acid",
    "fitik asit": "phytic acid",
    "glukonolakton": "gluconolactone",
    "laktobiyonik": "lactobionic acid",
    "süksinik asit": "succinic acid",
    "retinaldehit": "retinaldehyde",
    "retinil palmitat": "retinyl palmitate",
    "granactive": "hydroxypinacolone retinoate",
    "traneksamik": "tranexamic acid",
    "kojik": "kojic acid",
    "hidrokinon": "hydroquinone",
    "pantenol": "panthenol",
    "seramid": "ceramides",
    "üre": "urea",
    "çinko pca": "zinc pca",
    "kükürt": "sulfur",
    "benzoil peroksit": "benzoyl peroxide",
    "koenzim q10": "coenzyme q10",
    "ubikinon": "coenzyme q10",
    "ubiquinone": "coenzyme q10",
    "peptid": "peptide",
    "matrixyl": "peptide",
    "argireline": "peptide",
    "bakır peptid": "copper peptide",
    "ektoin": "ectoin",
    "c vitamini": "vitamin c",
    "askorbik": "ascorbic acid",
    "askorbil glukozit": "ascorbyl glucoside",
    "etil askorbik": "ethyl ascorbic acid",
    "e vitamini": "vitamin e",
    "tokoferol": "tocopherol",
    "a vitamini": "vitamin a",
    "b5 vitamini": "vitamin b5",
    "biyotin": "biotin",
    "ergotiyonein": "ergothioneine",
    "glutatyon": "glutathione",
    "hyaluronik": "hyaluronic acid",
    "gliserin": "glycerin",
    "betain": "betaine",
    "trehaloz": "trehalose",
    "propandiol": "propanediol",
    "skualan": "squalane",
    "skualen": "squalene",
    "vazelin": "petrolatum",
    "kolesterol": "cholesterol",
    "fitosfingozin": "phytosphingosine",
    "beta glukan": "beta glucan",
    "poliglutamik": "polyglutamic acid",
    "madecassosid": "madecassoside",
    "gotu kola": "centella",
    "cica": "centella",
    "çinko oksit": "zinc oxide",
    "titanyum dioksit": "titanium dioxide",
    "avobenzon": "avobenzone",
    "oktokrilen": "octocrylene",
    "homosalat": "homosalate",
    # oils
    "argan": "argan oil",
    "shea yağı": "shea butter",
    "kakao yağı": "cocoa butter",
    "badem yağı": "almond oil",
    "sweet almond": "almond oil",
    "hindistan cevizi yağı": "coconut oil",
    "zeytinyağı": "olive oil",
    "zeytin yağı": "olive oil",
    "ayçiçek yağı": "sunflower oil",
    "üzüm çekirdeği yağı": "grapeseed oil",
    "grapeseed": "grapeseed oil",
    "hint yağı": "castor oil",
    "susam yağı": "sesame oil",
    "avokado yağı": "avocado oil",
    "çuha çiçeği": "evening primrose oil",
    "evening primrose": "evening primrose oil",
    "hodan yağı": "borage oil",
    "kenevir yağı": "hemp seed oil",
    "aspir yağı": "safflower oil",
    "kamelya yağı": "camellia oil",
    "tamanu yağı": "tamanu oil",
    "marula yağı": "marula oil",
    "kuşburnu": "rosehip oil",
    "rosehip": "rosehip oil",
    "nar çekirdeği yağı": "pomegranate seed oil",
    "yalancı iğde": "sea buckthorn oil",
    "sea buckthorn": "sea buckthorn oil",
    "çay ağacı": "tea tree oil",
    "tea tree": "tea tree oil",
    "neem yağı": "neem oil",
    # essential oils
    "lavanta": "lavender oil",
    "lavender": "lavender oil",
    "biberiye": "rosemary oil",
    "rosemary": "rosemary oil",
    "nane yağı": "peppermint oil",
    "peppermint": "peppermint oil",
    "okaliptus": "eucalyptus oil",
    "eucalyptus": "eucalyptus oil",
    "sardunya": "geranium oil",
    "geranium": "geranium oil",
    "günlük yağı": "frankincense oil",
    "frankincense": "frankincense oil",
    "sandal ağacı": "sandalwood oil",
    "sandalwood": "sandalwood oil",
    "paçuli": "patchouli oil",
    "patchouli": "patchouli oil",
    "kekik yağı": "thyme oil",
    # extracts
    "aloe vera özü": "aloe vera",
    "papatya": "chamomile extract",
    "chamomile": "chamomile extract",
    "yeşil çay": "green tea extract",
    "green tea": "green tea extract",
    "meyan kökü": "licorice extract",
    "licorice": "licorice extract",
    "aynısefa": "calendula extract",
    "calendula": "calendula extract",
    "cadı fındığı": "witch hazel",
    "ginseng özü": "ginseng extract",
    "ginseng": "ginseng extract",
    "zerdeçal": "turmeric extract",
    "turmeric": "turmeric extract",
    "salatalık özü": "cucumber extract",
    "cucumber": "cucumber extract",
    "yulaf": "oat extract",
    "oat": "oat extract",
    "pirinç özü": "rice extract",
    "soya özü": "soy extract",
    "soybean": "soy extract",
    "söğüt kabuğu": "willow bark extract",
    "willow bark": "willow bark extract",
    "çam kabuğu": "pine bark extract",
    "pine bark": "pine bark extract",
    "üzüm çekirdeği özü": "grape seed extract",
    "nar özü": "pomegranate extract",
    "zencefil özü": "ginger extract",
    "at kestanesi": "horse chestnut extract",
    "salyangoz": "snail mucin",
    "snail": "snail mucin",
    "arı zehiri": "bee venom",
    "bal özü": "honey extract",
    # proteins / misc
    "kolajen": "collagen",
    "dimetikon": "dimethicone",
}


def _regex_extract_ingredients(text: str) -> list[str]:
    t = (text or "").lower()
    found = set()
    for term in _INGREDIENT_TERMS:
        # word-ish boundary for latin terms; for TR terms just substring is fine
        if any(c in term for c in (" ", "ç", "ğ", "ı", "ö", "ş", "ü")):
            if term in t:
                found.add(term)
        else:
            if re.search(rf"\b{re.escape(term)}\b", t):
                found.add(term)
    # normalize synonyms (TR + alternate spellings) to canonical-ish English names
    norm = []
    for x in sorted(found):
        norm.append(_NORMALIZE_MAP.get(x, x))
    # de-dup again after normalization
    out = []
    seen = set()
    for x in norm:
        if x not in seen:
            out.append(x)
            seen.add(x)
    return out[:24]


def _apply_regex_klass_batch(
    *,
    user_id: str,
    folder_id: str | None,
    batch: list[tuple],
) -> int:
    """
    Deterministic klass when Gemini is unavailable or returned invalid JSON.
    batch: list of (chunk_id, chunk_text).
    """
    n = 0
    with pg_conn(autocommit=True) as conn:
        with conn.cursor() as cur:
            for (cid, txt) in batch:
                ings = _regex_extract_ingredients(txt or "")
                k = {
                    "id": str(cid),
                    "topic": "ingredient" if ings else "general",
                    "ingredients": ings,
                    "evidence_type": "unknown",
                    "claims": [],
                    "language": "tr" if any(ch in (txt or "") for ch in "çğıöşüÇĞİÖŞÜ") else "en",
                    "method": "regex",
                }
                _exec(
                    cur,
                    """
                    update public.knowledge_chunks
                    set klass = %s::jsonb
                    where id = %s and user_id = %s
                    """,
                    (json.dumps(k, ensure_ascii=False), cid, user_id),
                )
                n += 1
                if ings:
                    try:
                        _upsert_entity_links(
                            user_id=user_id,
                            folder_id=str(folder_id) if folder_id else None,
                            chunk_id=str(cid),
                            ingredients=ings,
                        )
                    except Exception:
                        pass
    return n


def _parse_classifier_response(raw: str) -> list:
    """Parse JSON array from model output; strip optional markdown fences."""
    s = (raw or "").strip()
    if not s:
        return []
    if s.startswith("```"):
        s = re.sub(r"^```(?:json)?\s*", "", s, flags=re.I)
        s = re.sub(r"\s*```\s*$", "", s)
    try:
        parsed = json.loads(s)
    except json.JSONDecodeError:
        raise
    if not isinstance(parsed, list):
        raise json.JSONDecodeError("Classifier returned non-array JSON", s, 0)
    return parsed


def _collect_exception_texts(exc: BaseException) -> str:
    """Flatten exception + causes/contexts into one searchable blob."""
    parts: list[str] = []
    cur: BaseException | None = exc
    seen: set[int] = set()
    for _ in range(12):
        if cur is None or id(cur) in seen:
            break
        seen.add(id(cur))
        parts.append(str(cur))
        parts.append(repr(cur))
        for a in getattr(cur, "args", ()) or ():
            parts.append(str(a))
        nxt = getattr(cur, "__cause__", None) or getattr(cur, "__context__", None)
        cur = nxt if isinstance(nxt, BaseException) else None
    return "\n".join(parts)


def _looks_like_bad_json(e: BaseException) -> bool:
    """True if failure is JSON parse / truncated model output (including wrapped API errors)."""
    if isinstance(e, json.JSONDecodeError):
        return True
    blob = _collect_exception_texts(e)
    if "jsondecodeerror" in blob.lower():
        return True
    if any(
        frag in blob
        for frag in (
            "Unterminated string",
            "Expecting value",
            "Expecting ',' delimiter",
            "Invalid control character",
            "Invalid \\escape",
            "Expecting property name",
            "Extra data",
            "Classifier returned non-array",
        )
    ):
        return True
    # stdlib JSONDecodeError line/column pattern
    if "line " in blob and "column" in blob and ("char " in blob or "char)" in blob):
        return True
    return False


def _is_transient_db_or_network(e: BaseException) -> bool:
    """Long classify runs may hit brief Supabase or TLS blips; Gemini HTTP can time out too."""
    blob = _collect_exception_texts(e).lower()
    if "timeout" in blob and ("connection" in blob or "connect" in blob or "read" in blob):
        return True
    if "temporarily unavailable" in blob:
        return True
    if "server closed the connection" in blob or "connection reset" in blob:
        return True
    if "could not receive data from server" in blob or "could not send data to server" in blob:
        return True
    if "ssl" in blob and "error" in blob:
        return True
    return False


def _ensure_client():
    if not gemini_client:
        raise RuntimeError("Gemini client not ready; set GEMINI_API_KEY")
    return gemini_client


def _is_empty_klass(v) -> bool:
    if v is None:
        return True
    if isinstance(v, dict) and len(v.keys()) == 0:
        return True
    if isinstance(v, str) and v.strip() in {"", "{}", "null"}:
        return True
    return False


def classify_chunks(
    *,
    user_id: str,
    folder_slug: Optional[str] = None,
    document_id: Optional[str] = None,
    limit: int = 400,
    batch_size: int = 4,
    model: str = "gemini-2.0-flash",
    force: bool = False,
    upgrade_regex: bool = False,
) -> dict:
    """
    Fills `knowledge_chunks.klass` with lightweight scientific routing metadata:
      - topic: acne|pigmentation|barrier|sun|eczema|rosacea|hair|general|...
      - ingredients: ["niacinamide", "adapalene", ...]
      - evidence_type: rct|meta|review|mechanism|guideline|observational|case|unknown
      - claims: short bullet-like claims list
      - language: tr|en|other
    """
    client = None
    # Force deterministic mode (no Gemini) when quota is unavailable.
    if (os.getenv("KNOWLEDGE_CLASSIFY_MODE") or "").strip().lower() == "regex":
        client = None
    else:
        try:
            client = _ensure_client()
        except Exception:
            client = None

    updated = 0
    skipped = 0
    failed = 0

    with pg_conn(autocommit=True) as conn:
        with conn.cursor() as cur:
            folder_id = None
            if document_id:
                _exec(
                    cur,
                    """
                    select folder_id from public.knowledge_documents
                    where id = %s::uuid and user_id = %s
                    """,
                    (document_id, user_id),
                )
                drow = cur.fetchone()
                if not drow or not drow[0]:
                    raise RuntimeError(f"Document not found for user: {document_id}")
                folder_id = drow[0]
            if folder_slug:
                _exec(
                    cur,
                    "select id from public.knowledge_folders where user_id=%s and slug=%s",
                    (user_id, folder_slug),
                )
                row = cur.fetchone()
                fid = row[0] if row else None
                if not fid:
                    raise RuntimeError(f"Folder not found for slug={folder_slug}")
                if document_id and str(fid) != str(folder_id):
                    raise RuntimeError("--document-id belongs to a different folder than --folder")
                folder_id = fid

            # Without --force: only rows that still need klass (not "newest N rows then skip").
            # Otherwise older chunks with empty klass never appear when limit < total table size.
            _exec(
                cur,
                """
                select id, chunk_text, klass
                from public.knowledge_chunks
                where user_id = %s
                  and (%s::uuid is null or folder_id = %s::uuid)
                  and (%s::uuid is null or document_id = %s::uuid)
                  and embed_ok is true
                  and embedding is not null
                  and (
                    %s::boolean
                    or klass is null
                    or klass = '{}'::jsonb
                    or (
                      %s::boolean
                      and klass ? 'method'
                      and klass->>'method' = 'regex'
                    )
                  )
                order by created_at asc
                limit %s
                """,
                (
                    user_id,
                    folder_id,
                    folder_id,
                    document_id,
                    document_id,
                    bool(force),
                    bool(upgrade_regex),
                    max(int(limit), 1),
                ),
            )
            rows = cur.fetchall() or []

    # classify outside transaction loops (still updates inside new connections below)
    to_process = []
    for (chunk_id, chunk_text, klass) in rows:
        if not force:
            is_empty = _is_empty_klass(klass)
            is_regex = isinstance(klass, dict) and klass.get("method") == "regex"
            if not is_empty and not (upgrade_regex and is_regex):
                skipped += 1
                continue
        # Shorter text = smaller model JSON (fewer truncation / parse errors).
        to_process.append((chunk_id, (chunk_text or "")[:1800]))

    log.info(
        "Chunks: %d DB candidates, %d to classify, %d skipped",
        len(rows),
        len(to_process),
        skipped,
    )

    for i in range(0, len(to_process), batch_size):
        batch = to_process[i : i + batch_size]

        # If Gemini is unavailable or quota-limited, do deterministic regex ingredient extraction only.
        if client is None:
            updated += _apply_regex_klass_batch(
                user_id=user_id,
                folder_id=str(folder_id) if folder_id else None,
                batch=batch,
            )
            continue

        def _classifier_prompt(sub_batch: list[tuple]) -> str:
            payload = [{"id": str(cid), "text": text} for (cid, text) in sub_batch]
            return (
                "You are a scientific document chunk classifier for skincare/dermatology.\n"
                "For each chunk, produce a compact JSON classification.\n\n"
                "Output schema for each item:\n"
                "{\n"
                '  "id": "<uuid>",\n'
                '  "topic": "acne|pigmentation|barrier|sun|eczema|rosacea|hair|ingredient|general|other",\n'
                '  "ingredients": ["..."],\n'
                '  "evidence_type": "rct|meta|review|mechanism|guideline|observational|case|unknown",\n'
                '  "claims": ["short claim 1", "short claim 2"],\n'
                '  "language": "tr|en|other"\n'
                "}\n\n"
                "Rules:\n"
                "- Return ONLY JSON array.\n"
                "- Escape quotes and newlines inside JSON strings (valid JSON only).\n"
                "- Ingredients should be normalized to lowercase ASCII when possible (e.g., niacinamide, adapalene, azelaic acid).\n"
                "- Claims: 0-2 items, each under 120 characters.\n"
                "- If unsure, use topic=general and evidence_type=unknown.\n\n"
                f"Input JSON:\n{json.dumps(payload, ensure_ascii=False)}"
            )

        def try_batch(sub_batch: list[tuple], depth: int = 0, net_retries: int = 0) -> None:
            nonlocal updated, failed, client
            if not sub_batch:
                return
            if client is None:
                updated += _apply_regex_klass_batch(
                    user_id=user_id,
                    folder_id=str(folder_id) if folder_id else None,
                    batch=sub_batch,
                )
                return
            prompt = _classifier_prompt(sub_batch)
            try:
                resp = client.models.generate_content(
                    model=model,
                    contents=prompt,
                    config=types.GenerateContentConfig(
                        system_instruction="Return ONLY JSON. No markdown. Be conservative and accurate.",
                        temperature=0.2,
                        max_output_tokens=8192,
                        # Do not force application/json MIME: some stacks surface parse errors
                        # without chaining JSONDecodeError; we parse text ourselves.
                    ),
                )
                parsed = _parse_classifier_response(_gemini_response_text(resp) or "[]")

                id_to_klass = {}
                for item in parsed:
                    if not isinstance(item, dict):
                        continue
                    cid = item.get("id")
                    if cid:
                        id_to_klass[str(cid)] = item

                # Gemini is already done; Postgres connect can flake — retry DB without
                # double-counting updates (one transaction per attempt for klass writes).
                for db_attempt in range(10):
                    u_delta = 0
                    f_delta = 0
                    applied: list[tuple[str, dict]] = []
                    try:
                        with pg_conn(autocommit=False) as conn:
                            with conn.cursor() as cur:
                                for (cid, _txt) in sub_batch:
                                    k = id_to_klass.get(str(cid))
                                    if not k:
                                        f_delta += 1
                                        continue
                                    _exec(
                                        cur,
                                        """
                                        update public.knowledge_chunks
                                        set klass = %s::jsonb
                                        where id = %s and user_id = %s
                                        """,
                                        (json.dumps(k, ensure_ascii=False), cid, user_id),
                                    )
                                    u_delta += 1
                                    applied.append((str(cid), k))
                            conn.commit()
                        updated += u_delta
                        failed += f_delta
                        for cid_s, k in applied:
                            try:
                                ings = k.get("ingredients") if isinstance(k, dict) else None
                                if isinstance(ings, list) and ings:
                                    _upsert_entity_links(
                                        user_id=user_id,
                                        folder_id=str(folder_id) if folder_id else None,
                                        chunk_id=cid_s,
                                        ingredients=[str(x) for x in ings if x],
                                    )
                            except Exception:
                                pass
                        break
                    except Exception as db_e:
                        if db_attempt < 9 and _is_transient_db_or_network(db_e):
                            wait = min(6 + db_attempt * 7, 60)
                            log.warning(
                                "DB write transient (attempt %s/10); sleep %ss: %s",
                                db_attempt + 1,
                                wait,
                                db_e,
                            )
                            time.sleep(wait)
                            continue
                        raise
            except Exception as e:
                msg = str(e)
                quota = "RESOURCE_EXHAUSTED" in msg or "429" in msg
                if quota:
                    log.error(
                        "Classification quota/rate limit (batch %d..%d): %s",
                        i,
                        i + len(batch) - 1,
                        e,
                    )
                    client = None
                    updated += _apply_regex_klass_batch(
                        user_id=user_id,
                        folder_id=str(folder_id) if folder_id else None,
                        batch=sub_batch,
                    )
                    return
                if _looks_like_bad_json(e):
                    if len(sub_batch) > 1 and depth < 8:
                        mid = (len(sub_batch) + 1) // 2
                        log.warning(
                            "Classifier JSON invalid; splitting batch %d -> %d + %d (depth %d)",
                            len(sub_batch),
                            mid,
                            len(sub_batch) - mid,
                            depth,
                        )
                        try_batch(sub_batch[:mid], depth + 1, 0)
                        try_batch(sub_batch[mid:], depth + 1, 0)
                        return
                    log.warning(
                        "Classifier JSON invalid; regex fallback for %d chunk(s) at offset %d",
                        len(sub_batch),
                        i,
                    )
                    updated += _apply_regex_klass_batch(
                        user_id=user_id,
                        folder_id=str(folder_id) if folder_id else None,
                        batch=sub_batch,
                    )
                    return
                if _is_transient_db_or_network(e) and net_retries < 10:
                    wait = min(10 + net_retries * 8, 60)
                    log.warning(
                        "Transient DB/network error; retry %s/%s in %ss (%d..%d): %s",
                        net_retries + 1,
                        10,
                        wait,
                        i,
                        i + len(batch) - 1,
                        e,
                    )
                    time.sleep(wait)
                    try_batch(sub_batch, depth, net_retries + 1)
                    return
                log.error(
                    "Classification batch failed (%d..%d): %s",
                    i,
                    i + len(batch) - 1,
                    e,
                )
                failed += len(sub_batch)

        try_batch(batch, 0, 0)

    return {
        "user_id": user_id,
        "folder_slug": folder_slug,
        "document_id": document_id,
        "model": model,
        "upgrade_regex": bool(upgrade_regex),
        "total_scanned": len(rows),
        "classified_updated": updated,
        "skipped_existing": skipped,
        "failed": failed,
    }


if __name__ == "__main__":
    import argparse

    ap = argparse.ArgumentParser()
    ap.add_argument("--user", required=True, help="supabase auth user uuid")
    ap.add_argument("--folder", default=None, help="folder slug (optional)")
    ap.add_argument(
        "--document-id",
        default=None,
        dest="document_id",
        help="only classify chunks for this knowledge_documents.id (e.g. after single-file ingest)",
    )
    ap.add_argument("--limit", type=int, default=400)
    ap.add_argument("--batch", type=int, default=4, help="Gemini batch size (smaller = more stable JSON)")
    ap.add_argument("--model", default="gemini-2.0-flash")
    ap.add_argument("--force", action="store_true", help="overwrite existing klass and rebuild entity links")
    ap.add_argument(
        "--upgrade-regex",
        action="store_true",
        help="re-run Gemini for chunks that only have klass.method=regex (JSON fallback rows)",
    )
    args = ap.parse_args()

    result = classify_chunks(
        user_id=args.user,
        folder_slug=args.folder,
        document_id=args.document_id,
        limit=args.limit,
        batch_size=args.batch,
        model=args.model,
        force=bool(args.force),
        upgrade_regex=bool(args.upgrade_regex),
    )
    print(json.dumps(result, indent=2, ensure_ascii=False))

